...
app.post("/api/employees", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [admin_id, admin_fname, admin_lname, admin_email, bcryptPassword, role_id]
    );
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    do {
      client_id =
        "CL" + Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000;
...
app.post("/api/adds_on", async (req, res) => {
  try {
    ...
    const result = await pool.query(query, [id, adds_on_name, reservation_id]);
